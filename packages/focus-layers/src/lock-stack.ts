...
export class LockStack {
	 ...
	 * below to be disabled before this lock's callback is enabled. Locks should
	 * not enable themselves until the callback has been invoked to ensure that
	 * other locks are not also concurrently active.
	 */
...
export interface Lock {
	 ...
	 * Using asynchronous logic can cause issues where two mutually-exclusive
	 * locks may be active at the same time, causing an infinite loop of focus
	 * movement.
	 */
 ...
 * The first argument, `enabled`, indicates the enabled state of the top lock in
 * the stack, and the second argument, `stack`, is a reference to that stack
 * itself.
 */
