...
import { PATHS, NPM_SCOPE, PROJECT_NAME } from "./constants";
...
const rootTsConfig: TSConfigJSON = fs.readJSONSync(
	path.join(PATHS.PROJECT_ROOT, "tsconfig.json")
);
...
export async function buildAllPackages() {
	let start = Date.now();
	let packages = (await loadPackages()).filter(
		(pkg) => pkg.name !== `${NPM_SCOPE}/docs`
	);
	let topoPackageMap = await getPackageDirectoryMap();

	// Clean up old dist files before starting.
	await cleanDistDirectories();
	logBuildStepCompletion(PROJECT_NAME, "Cleaned up old files");
	...
	logBuildStepCompletion(
		PROJECT_NAME,
		`Whew! Finished build in ${timeFromStart(start)}`,
		"⏰"
	);
}

export async function buildPackage(packageName: string, packagePath: string) {
	let packageRoot = path.resolve(PATHS.PROJECT_ROOT, packagePath, "..");
	let filesInPackage = await fs.readdir(packagePath);
	let input: string[] = [];
	let extensions = new Set([".js", ".jsx", ".ts", ".tsx"]);
	for (let filename of filesInPackage) {
		if (extensions.has(path.extname(filename))) {
			input.push(path.join(packagePath, filename));
		}
	}
...
export async function build(packageDetails: {
	...
	try {
		for (let filename of opts.input) {
			await writeCjsEntryFile(getBasename(filename), opts.packageDist);
		}
		...
		await new Promise((done) => {
			buildConfigs.forEach(async (inputOptions, index, src) => {
				...
				if (index === src.length - 1) done(null);
			});
		});

		logBuildStepCompletion(
			opts.name,
			`Built modules in ${timeFromStart(start)}`,
			"⏰"
		);
...
async function createRollupConfig(
	...
	let fileBaseName = getBasename(input);
	...
	if (tsconfig) {
		try {
			...
			warn(
				name,
				"Local tsconfig JSON not found. Using the root project config instead."
			);
		}
	}
	...
	const includeTypeDeclaration = buildCount === 0;
	...
	return {
		...
		output: {
			...
			esModule:
				(tsconfigJSON && tsconfigJSON.compilerOptions.esModuleInterop) || false,
		},
		plugins: [
			...
			typescriptRollup({
				...
				tsconfigOverride: {
					...
					include: [
						...
						path.resolve(PATHS.PROJECT_ROOT, "types"),
					],
					compilerOptions: {
						outDir,
						declaration: includeTypeDeclaration,
						declarationMap: includeTypeDeclaration,
						declarationDir: includeTypeDeclaration ? outDir : undefined,
					},
				},
...
async function createBuildConfigs(
	...
	let allInputs = flatMap(opts.input, (input: string) => {
		return createAllFormats(opts, input).map<ScriptOpts>((options, index) => ({
			...options,
...
export async function getPackageDirectoryMap() {
	...
	for (let pkg of pkgs) {
		...
		if (
			...
			!rootTsConfig.compilerOptions.paths![pkg.name]
		) {
			continue;
		}

		topoPackageMap[pkg.name] = rootTsConfig.compilerOptions.paths![
			pkg.name
		]?.[0];
	}
...
if (process.env.NODE_ENV === 'production') {
  ...
  module.exports = require('./${name}.cjs.development.js');
	...
	return fs.outputFile(path.join(packageDist, `${name}.js`), contents);
}
